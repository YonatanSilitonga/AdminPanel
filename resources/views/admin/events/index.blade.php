...
<div id="event-manager" x-data="{ 
    ...
    async submitUpdate() {
        ...
        if (!eventId) {
            window.showAlert('ID Event tidak ditemukan', 'Perhatian', 'warning');
            this.loading = false;
            return;
        }
        ...
        try {
            ...
            // Set schedules and operational hours
            ...
            if (signData.success && signData.mode === 'cloudinary') {
                ...
                if (thumbnailInput && thumbnailInput.files.length > 0) {
                    ...
                    this.uploadProgressText = 'Menghubungkan ke Cloudinary untuk mengunggah banner...';
                    this.uploadSpeedText = '';
                    const res = await this.uploadToCloudinaryDirectly(thumbnailInput.files[0], signData);
                    formData.set('banner', res.secure_url);
                }
                ...
                const result = await this.uploadToLocalWithProgress(formData, `/admin/events/${eventId}`);
                this.uploadProgressPercent = 100;
    ...
    async submitCreate() {
        ...
        try {
            ...
            if (signData.success && signData.mode === 'cloudinary') {
                // Upload banner/thumbnail to Cloudinary if selected
                if (thumbnailInput && thumbnailInput.files.length > 0) {
                    this.showUploadProgress = true;
                    this.uploadProgressPercent = 0;
                    this.uploadProgressText = 'Menghubungkan ke Cloudinary untuk mengunggah banner...';
                ...
                const result = await this.uploadToLocalWithProgress(formData, '/admin/events');
                this.uploadProgressPercent = 100;
                this.uploadProgressText = 'Berhasil disimpan!';
                await new Promise(r => setTimeout(r, 500));
                this.showUploadProgress = false;
                ...
                if (result.success) {
                    ...
                    window.showAlert(result.message || 'Gagal menyimpan event', 'Gagal', 'error');
                }
            }
        } catch (error) {
            console.error(error);
    ...
    <button type="button" class="hidden" data-open-create-modal @click="showCreateModal = true" @open-create-modal.window="showCreateModal = true; createFileName = ''; createSchedule = []; openTime = '08:00'; closeTime = '17:00'; is_active = true;"></button>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 py-8">
            ...
            <template x-if="showCreateModal">
                <div x-show="showCreateModal"
                    ...
                    <div>
                        <form id="createEventForm" @submit.prevent="submitCreate()" action="{{ route('admin.events.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                            @csrf
                            <div class="space-y-2">
                                <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest">Nama Event</label>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div class="space-y-2" x-data="{ thumbPreview: '' }">
                                    ...
                                    <div class="relative group">
                                        <input type="file" name="banner" id="create_thumbnail" class="hidden" 
                                               @change="
