        ...
        <div class="bg-white rounded-[2rem] shadow-xl border border-gray-100 overflow-hidden">
            ...
            <div class="p-8">
                ...
                <div class="text-center mb-8">
                    <div class="inline-flex items-center justify-center w-24 h-24 rounded-2xl bg-danger/10 mb-4">
                        <svg class="w-12 h-12 text-danger" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
                        </svg>
                    </div>
                    <h1 class="text-4xl font-black text-gray-900">403</h1>
                </div>

                <!-- Error Message -->
                <div class="text-center mb-8">
                    <h2 class="text-2xl font-bold text-gray-900 mb-3">Akses Ditolak</h2>
                    <p class="text-gray-600 leading-relaxed">{{ $message ?? 'Anda tidak memiliki izin untuk mengakses sumber daya ini.' }}</p>
                </div>

                <!-- Additional Info -->
                @if(isset($reason))
                    <div class="mb-8 p-4 bg-danger/5 rounded-xl border border-danger/20">
                        <p class="text-sm text-gray-700"><span class="font-bold text-danger">⚠️ Alasan:</span> {{ $reason }}</p>
                    </div>
                @else
                    <div class="mb-8 p-4 bg-danger/5 rounded-xl border border-danger/20">
                        <p class="text-sm text-gray-700"><span class="font-bold text-danger">⚠️ Info:</span> Peran atau izin Anda tidak cukup untuk akses ini.</p>
                    </div>
                @endif
